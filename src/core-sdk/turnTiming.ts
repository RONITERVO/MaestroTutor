...
export interface TurnTimingStorage {
...
/** One recorder per client; browser storage and lifecycle wiring belong to its adapter. */
export function createTurnTimingRecorder(options: { storage?: () => TurnTimingStorage | undefined } = {}) {
  const reports: TurnTimingReport[] = [];
  let loaded = false;
  let pendingSave: ReturnType<typeof setTimeout> | undefined;

  function clearTurnTimings() {
    load();
    reports.length = 0;
    flushTurnTimings();
  }
  ...
  function load() {
    ...
    loaded = true;
    try {
      const saved: unknown = JSON.parse(options.storage?.()?.getItem(STORAGE_KEY) || '[]');
  ...
  function flushTurnTimings() {
    load();
    if (pendingSave !== undefined) clearTimeout(pendingSave);
    pendingSave = undefined;
    try { options.storage?.()?.setItem(STORAGE_KEY, JSON.stringify(reports)); } catch { /* Storage may be unavailable. */ }
  }

  function exportTurnTimings(): string {
    load();
  ...
  function beginTurnTiming(turnId: string, now = () => performance.now()) {
    load();
    const start = now();
  ...
  return { beginTurnTiming, exportTurnTimings, clearTurnTimings, flushTurnTimings };
}
