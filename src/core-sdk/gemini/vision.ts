...
import type { UsageCostInput } from '../../../shared/pricing/usage';
import type { GeminiClientSource } from './clientSource';

export type ImageUsageSink = (usage: UsageCostInput & { feature: 'image' }) => void;
...
export type GenerateImageParams = GeminiClientSource & {
  prompt?: string;
  history?: any[];
...
export const generateImage = async (params: GenerateImageParams): Promise<ImageGenerationResult> => {
  const ai = params.aiClient || await params.resolveAiClient();
