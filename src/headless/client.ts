...
export const describeHeadlessMethods = (accessMode: HeadlessAccessMode = 'managed') => {
  const methodInfo = {
    ...
    'speech.synthetic.live': { mutates: true, params: ['pcmBase64', 'sampleRate?', 'chunkDurationMs?', 'pace?', 'connectedTurns? (1..4, same PCM over one socket)', 'systemInstruction?', 'model?', 'gateInputOnSpeech?', 'semanticSpeech?', 'simulateUiSpeechHandoff?', 'requireRealtimeInputPacing?', 'playModelAudioRealtime?', 'timeoutMs?', 'includeModelAudio?'] },
