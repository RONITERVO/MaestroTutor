// Copyright 2025 Roni Tervo
//
// SPDX-License-Identifier: Apache-2.0
export const taTranslations: Record<string, string> = {
  // App title
  "app.title": "Maestro (மேஸ்ட்ரோ)",
  // Language selector (used in SttLanguageSelector)
  "sttLang.selectLanguage": "பேச்சு அங்கீகாரத்தை {language} க்கு அமைக்கவும்",

  // Header
  "header.targetLanguageTitle": "தற்போதைய இலக்கு மொழி: {language}",

  // Start page (used)
  "startPage.clickToStart": "விமானத்தைக் கிளிக் செய்யவும்",
  "startPage.saveChats": "அனைத்து அரட்டைகளையும் சேமிக்கவும்",
  "startPage.loadChats": "அரட்டைகளை ஏற்றவும்",
  "startPage.saveThisChat": "இந்த அரட்டையைச் சேமிக்கவும்",
  "startPage.appendToChat": "அரட்டையில் சேர்க்கவும்",
  "startPage.trimBeforeBookmark": "புக்மார்க்கிற்கு முன் உள்ளவற்றை நீக்கவும்",
  "startPage.maestroAvatar": "மேஸ்ட்ரோ அவதாரம்",
  "startPage.addMaestroAvatar": "மேஸ்ட்ரோ அவதாரம் சேர்க்கவும்",
  "startPage.loadSuccess": "{count} அரட்டை அமர்வுகள் வெற்றிகரமாக ஏற்றப்பட்டு மாற்றப்பட்டன!",
  "startPage.loadError": "அரட்டைகளை ஏற்றுவதில் பிழை. கோப்பு சிதைந்திருக்கலாம் அல்லது தவறான வடிவத்தில் இருக்கலாம்.",
  "startPage.noChatsToSave": "சேமிக்க அரட்டை வரலாறு இல்லை.",
  "startPage.saveError": "அரட்டைகளைச் சேமிப்பதில் பிழை. மேலும் விவரங்களுக்கு கன்சோலைப் பார்க்கவும்.",
  "startPage.noChatSelected": "தயவுசெய்து முதலில் மொழி ஜோடியைத் தேர்ந்தெடுக்கவும்.",
  "startPage.noBookmarkSet": "புக்மார்க் அமைக்கப்படவில்லை. செய்திகளை நீக்க முதலில் புக்மார்க்கை அமைக்கவும்.",
  "startPage.noMessagesToTrim": "நீக்குவதற்கு புக்மார்க்கிற்கு முன் செய்திகள் எதுவுமில்லை.",
  "startPage.trimSuccess": "புக்மார்க்கிற்கு முன் இருந்த {count} செய்திகள் நீக்கப்பட்டன.",
  "startPage.trimError": "செய்திகளை நீக்குவதில் தோல்வி. மீண்டும் முயற்சிக்கவும்.",
  "startPage.noMessagesToAppend": "காப்புப்பிரதி கோப்பில் சேர்ப்பதற்கான செய்திகள் எதுவும் இல்லை.",
  "startPage.noPairInBackup": "காப்புப்பிரதி கோப்பில் உங்களின் தற்போதைய மொழி ஜோடிக்கான செய்திகள் இல்லை. உங்கள் தற்போதைய அரட்டைக்கு பொருந்தும் காப்புப்பிரதியைத் தேர்ந்தெடுக்கவும்.",
  "startPage.appendSuccess": "தற்போதைய அரட்டையில் {count} செய்திகள் வெற்றிகரமாகச் சேர்க்கப்பட்டன.",
  "startPage.combineSuccess": "அரட்டைகள் இணைக்கப்பட்டன: {added} புதிய செய்திகள் சேர்க்கப்பட்டன, மொத்தம் {total} செய்திகள்.",
  "startPage.combineNoDuplicates": "அனைத்து செய்திகளும் ஏற்கனவே உங்கள் அரட்டையில் உள்ளன. மாற்றங்கள் எதுவும் செய்யப்படவில்லை.",
  "startPage.combineNoChanges": "சேர்ப்பதற்குப் புதிய செய்திகள் எதுவும் இல்லை. உங்கள் அரட்டையில் மாற்றமில்லை.",
  "startPage.invalidBackupFormat": "தவறான காப்புப்பிரதி கோப்பு. சரியான மேஸ்ட்ரோ காப்புப்பிரதி (.ndjson) கோப்பைத் தேர்ந்தெடுக்கவும்.",
  "startPage.browserNotSupported": "உங்கள் உலாவி கோப்புச் சேமிப்பை ஆதரிக்கவில்லை. தயவுசெய்து Chrome அல்லது Edge ஐப் பயன்படுத்தவும்.",

  // Session Controls - Action labels and descriptions
  "sessionControls.saveAll.label": "அனைத்தையும் சேமி",
  "sessionControls.saveAll.description": "அனைத்து அரட்டைகளையும் காப்புப்பிரதி கோப்பிற்கு ஏற்றுமதி செய்யவும்",
  "sessionControls.loadAll.label": "அனைத்தையும் ஏற்றவும்",
  "sessionControls.loadAll.description": "அனைத்து அரட்டைகளையும் காப்புப்பிரதி கோப்பைக் கொண்டு மாற்றவும்",
  "sessionControls.reset.label": "மீட்டமை",
  "sessionControls.reset.description": "அனைத்துத் தரவையும் காப்புப் பிரதி எடுத்து நீக்கவும்",
  "sessionControls.saveThis.label": "அரட்டையைச் சேமி",
  "sessionControls.saveThis.description": "இந்த அரட்டையை மட்டும் ஏற்றுமதி செய்யவும்",
  "sessionControls.combine.label": "இணைக்கவும்",
  "sessionControls.combine.description": "காப்புப்பிரதியை இந்த அரட்டையுடன் இணைக்கவும்",
  "sessionControls.trim.label": "கத்தரிக்கவும்",
  "sessionControls.trim.description": "புக்மார்க்கிற்கு முன் உள்ள செய்திகளை நீக்கவும்",

  // Session Controls - UI elements
  "sessionControls.profile": "சுயவிவரம்:",
  "sessionControls.profilePlaceholder": "உங்கள் பெயர் அல்லது விவரங்கள்...",
  "sessionControls.editProfile": "பயனர் சுயவிவரத்தைத் திருத்தவும்",
  "sessionControls.allChatsControls": "அனைத்து அரட்டை கட்டுப்பாடுகள்",
  "sessionControls.thisChatsControls": "இந்த அரட்டை கட்டுப்பாடுகள்",
  "sessionControls.back": "பின்செல்",
  "sessionControls.backupAndReset": "காப்புப்பிரதி மற்றும் மீட்டமைப்பு",
  "sessionControls.typeToConfirm": "உறுதிப்படுத்த \"{keyword}\" என தட்டச்சு செய்யவும்",
  "sessionControls.changeAvatar": "அவதாரத்தை மாற்றவும்",

  // General
  "general.clear": "அழி",
  "general.error": "மன்னிக்கவும், நான் ஒரு பிழையை எதிர்கொண்டேன்.",

  // API key gate
  "apiKeyGate.title": "உங்கள் Gemini API சாவியை இணைக்கவும்",
  "apiKeyGate.billingTitle": "அதிக ஒதுக்கீட்டிற்கு பில்லிங் அமைக்கவும்",
  "apiKeyGate.infoLogin": "உங்கள் உள்நுழைவு: ஒரு API விசை",
  "apiKeyGate.infoVisibility": "யார் பார்ப்பது: நீங்கள் மட்டுமே",
  "apiKeyGate.infoBilling": "பில்லிங்: மாதந்தோறும், Google இலிருந்து",
  "apiKeyGate.infoCost": "செலவு: பயன்பாட்டில் காட்டப்பட்டுள்ளது — பார்க்கத் தட்டவும்",
  "apiKeyGate.infoMore": "மேலும் தகவல்:",
  "apiKeyGate.privacyPolicy": "தனியுரிமைக் கொள்கை",
  "apiKeyGate.stepsTitle": "இரண்டு விரைவான படிகள்:",
  "apiKeyGate.stepOne": "Google AI Studio-ஐத் திறந்து API சாவியை உருவாக்கவும்.",
  "apiKeyGate.stepTwo": "கீழே சாவியை ஒட்டி சேமி (Save) என்பதைத் தொடவும்.",
  "apiKeyGate.openAiStudio": "Google AI Studio-ஐத் திறக்கவும்",
  "apiKeyGate.viewInstructions": "வழிமுறைகளைப் பார்க்கவும்",
  "apiKeyGate.closeInstructions": "வழிமுறைகளை மூடவும்",
  "apiKeyGate.previousInstruction": "முந்தைய வழிமுறை",
  "apiKeyGate.nextInstruction": "அடுத்த வழிமுறை",
  "apiKeyGate.instructionStep": "வழிமுறை {step} / {total}",
  "apiKeyGate.keyLabel": "Gemini API சாவி",
  "apiKeyGate.placeholder": "உங்கள் API சாவியை இங்கே ஒட்டவும்",
  "apiKeyGate.show": "காண்பி",
  "apiKeyGate.hide": "மறை",
  "apiKeyGate.currentKeySaved": "தற்போதைய சாவி சேமிக்கப்பட்டது {maskedKey}",
  "apiKeyGate.keyInvalid": "சாவி செல்லாதது {maskedKey} — கீழே புதிய சாவியை ஒட்டவும்",
  "apiKeyGate.clearSavedKey": "சேமித்த சாவியை அழி",
  "apiKeyGate.cancel": "ரத்துசெய்",
  "apiKeyGate.saving": "சேமிக்கிறது...",
  "apiKeyGate.saveKey": "சாவியைச் சேமி",
  "apiKeyGate.close": "மூடு",
  "apiKeyGate.costLabel": "மதிப்பிடப்பட்ட API செலவு",
  // API key gate - tester form
  "apiKeyGate.testerFormTitle": "செயலியைச் சோதிக்கவும்",
  "apiKeyGate.testerFormDescription": "எங்கள் Android பீட்டா அழைப்பைப் பெற, Google Play Store-க்கு நீங்கள் பயன்படுத்தும் மின்னஞ்சலை உள்ளிடவும்!",
  "apiKeyGate.testerFormSubmit": "ஏர்லி அக்சஸ் பெறவும்",
  "apiKeyGate.testerFormSubmitting": "அனுப்பப்படுகிறது...",
  "apiKeyGate.testerFormError": "ஏதோ தவறு நடந்துள்ளது. மீண்டும் முயற்சிக்கவும்.",
  "apiKeyGate.testerFormMustBeGmail": "Google Play-க்கு @gmail.com முகவரியைப் பயன்படுத்தவும்.",
  "apiKeyGate.testerFormCapReached": "சமர்ப்பிப்பு வரம்பு முடிந்தது. நன்றி!",
  "apiKeyGate.developerLogin": "டெவலப்பர் உள்நுழைவு",
  "apiKeyGate.submitAnotherEmail": "தவறான மின்னஞ்சலை உள்ளிட்டீர்களா? மீட்டமைத்து இன்னொன்றை முயற்சிக்கவும்.",
  "apiKeyGate.testerFormChecking": "உங்கள் அழைப்பு நிலையைச் சரிபார்க்கிறது...",
  "apiKeyGate.testerFormApprovedTitle": "நீங்கள் இணைந்துவிட்டீர்கள்! 🎉",
  "apiKeyGate.testerFormApprovedDesc": "Android பீட்டா சோதனைத் திட்டத்திற்கு உங்கள் மின்னஞ்சல் அங்கீகரிக்கப்பட்டது.",
  "apiKeyGate.testerFormDownloadBtn": "Google Play-இல் செயலியைப் பதிவிறக்கவும்",
  "apiKeyGate.testerFormPendingTitle": "நீங்கள் காத்திருப்போர் பட்டியலில் உள்ளீர்கள்!",
  "apiKeyGate.testerFormPendingDesc": "உங்கள் மின்னஞ்சலைப் பெற்றுள்ளோம். அழைப்பைச் செயல்படுத்த சிறிது நேரம் ஆகும். சிறிது நேரம் கழித்துச் சரிபார்க்கவும்!",
  "apiKeyGate.checkStatusModeBtn": "ஏற்கனவே விண்ணப்பித்துவிட்டீர்களா? நிலையைச் சரிபார்க்கவும்.",
  "apiKeyGate.submitModeBtn": "விண்ணப்பிக்க வேண்டுமா? இங்கே பதிவு செய்யவும்.",
  "apiKeyGate.checkStatusTitle": "அழைப்பு நிலையைச் சரிபார்க்கவும்",
  "apiKeyGate.checkStatusDesc": "காத்திருப்போர் பட்டியலுக்கு நீங்கள் பயன்படுத்திய மின்னஞ்சலை உள்ளிடவும்.",
  "apiKeyGate.checkStatusSubmit": "நிலையைச் சரிபார்க்கவும்",
  "apiKeyGate.testerFormNotFound": "காத்திருப்போர் பட்டியலில் மின்னஞ்சல் இல்லை. முதலில் பதிவு செய்யவும்.",
  
  // Chat - general
  "chat.thinking": "யோசிக்கிறேன்...",
  "chat.loadingHistory": "அரட்டை வரலாற்றை ஏற்றுகிறது...",
  "chat.loadingSuggestions": "பரிந்துரைகளை ஏற்றுகிறது...",
  "chat.suggestionsAriaLabel": "பதில் பரிந்துரைகள்",
  "chat.attachImageFromFile": "கோப்பை இணைக்கவும்",
  "chat.removeAttachedImage": "இணைக்கப்பட்ட கோப்பை நீக்கவும்",
  "chat.sendMessage": "செய்தி அனுப்பவும்",
  "chat.messageInputAriaLabel": "செய்தி உள்ளீடு",
  "chat.retrievedFromWeb": "வலையிலிருந்து பெறப்பட்டது:",
  "chat.videoNotSupported": "உங்கள் உலாவி வீடியோ டேக்கை ஆதரிக்கவில்லை.",
  "chat.audioNotSupported": "உங்கள் உலாவி ஆடியோ டேக்கை ஆதரிக்கவில்லை.",
  "chat.fileAttachment": "கோப்பு இணைப்பு",
  "chat.pdf.loading": "PDF ஏற்றப்படுகிறது...",
  "chat.pdf.error": "PDF ஐ காண்பிக்க இயலவில்லை",
  "chat.pdf.pageIndicator": "பக்கம் {current} / {total}",
  "chat.imageGenError": "படம் உருவாக்குவதில் பிழை",
  "chat.generatingImageLoadingSlow": "சிறிது அதிக நேரம் எடுக்கிறது...",
  "chat.stopSpeaking": "பேசுவதை நிறுத்தவும்",
  "chat.speakThisLine": "இந்த வரியைப் பேசவும்",
  "chat.languageSelector.openGlobe": "மொழிகளை மாற்றவும்",
  "chat.maestroTranscriptScrollwheel": "மேஸ்ட்ரோ டிரான்ஸ்கிரிப்ட் ஸ்க்ரோல் பார்வை",

  // Chat - mic/STT
  "chat.mic.listening": "STT செயலில்: கேட்கிறேன்...",
  "chat.mic.enableStt": "STT ஐ இயக்கவும்",
  "chat.mic.disableStt": "STT ஐ நிறுத்தவும்",
  "chat.mic.recordingAudioNote": "ஆடியோ பதிவு செய்கிறது...",

  // Chat - placeholders
  "chat.placeholder.normal.listening": "{language} இல் கேட்கிறேன்...",
  "chat.placeholder.normal.sttActive": "{language} இல் பேசவும் அல்லது தட்டச்சு செய்யவும்...",
  "chat.placeholder.normal.sttInactive": "தட்டச்சு செய்யவும் அல்லது {language} இல் பேச மைக்கைத் தொடவும்...",
  "chat.placeholder.suggestion.listening": "மொழிபெயர்க்க {language} பேசவும்...",
  "chat.placeholder.suggestion.sttActive": "மொழிபெயர்க்க {language} இல் பேசவும் அல்லது தட்டச்சு செய்யவும்...",
  "chat.placeholder.suggestion.sttInactive": "மொழிபெயர்க்க {language} இல் தட்டச்சு செய்யவும்...",

  // Chat - camera
  "chat.camera.turnOn": "கேமரா முன்னோட்டத்தை இயக்கவும்",
  "chat.camera.turnOff": "கேமரா முன்னோட்டத்தை முடக்கவும்",
  "chat.camera.selectCamera": "கேமராவைத் தேர்ந்தெடுக்கவும்",
  "chat.camera.imageGenCameraLabel": "படம் உருவாக்கம்",
  "chat.camera.captureOrRecord": "படத்திற்கு தொடவும், வீடியோவிற்கு அழுத்திப் பிடிக்கவும்",
  "chat.camera.stopRecording": "பதிவை நிறுத்தவும்",
  "chat.bookIcon.toggleImageGen": "படம் உருவாக்கும் பயன்முறையை மாற்றவும்",

  // Chat - image
  "chat.imagePreview.alt": "முன்னோட்டம்",
  "chat.image.dragToEnlarge": "பெரிதாக்க மூலையை இழுக்கவும்",
  "chat.image.dragToShrink": "சிறிதாக்க மூலையை இழுக்கவும்",
  "chat.annotateImage": "படத்தில் குறிப்பு சேர்க்கவும்",
  "chat.annotateVideoFrame": "தற்போதைய ஃப்ரேமில் குறிப்பு சேர்க்கவும்",

  // Chat - annotate modal
  "chat.annotateModal.editingPreviewAlt": "குறிப்பு சேர்க்க வேண்டிய படம்",
  "chat.annotateModal.cancel": "ரத்துசெய்",
  "chat.annotateModal.saveAndAttach": "சேமித்து இணைக்கவும்",
  "chat.annotateModal.undo": "செயல்தவிர்",

  // Chat - suggestions
  "chat.suggestion.speak": "சொல்லவும்: \"{suggestion}\"",
  "chat.suggestion.ariaLabel": "பரிந்துரையைப் பேசவும்: {suggestion}",
  "chat.suggestion.toggleCreateMode": "பரிந்துரை உருவாக்கும் பயன்முறையை மாற்றவும்",
  "chat.suggestion.createAction": "பரிந்துரை உருவாக்கவும்",
  "chat.suggestion.creating": "பரிந்துரை உருவாகிறது...",

  // Chat - maestro status
  "chat.maestro.idle": "மேஸ்ட்ரோ செயலற்றது",
  "chat.maestro.title.idle": "மேஸ்ட்ரோ தற்போது செயலற்றது.",
  "chat.maestro.resting": "மேஸ்ட்ரோ ஓய்வெடுக்கிறது...",
  "chat.maestro.observing": "மேஸ்ட்ரோ கவனித்துக்கொண்டிருக்கிறது...",
  "chat.maestro.aboutToEngage": "மேஸ்ட்ரோ விரைவில் இணையும்...",
  "chat.maestro.title.resting": "மேஸ்ட்ரோ செயலற்றது, மீண்டும் இணைய நிறைய நேரம் உள்ளது.",
  "chat.maestro.title.observing": "மேஸ்ட்ரோ கவனித்துக்கொண்டிருக்கிறது, மீண்டும் இணைய சிறிது நேரம் உள்ளது.",
  "chat.maestro.title.aboutToEngage": "மேஸ்ட்ரோ விரைவில் மீண்டும் இணையும்.",
  "chat.maestro.typing": "மேஸ்ட்ரோ தட்டச்சு செய்கிறது...",
  "chat.maestro.title.typing": "மேஸ்ட்ரோ பதிலை தயாரிக்கிறது.",
  "chat.maestro.speaking": "மேஸ்ட்ரோ பேசுகிறது",
  "chat.maestro.title.speaking": "மேஸ்ட்ரோ இப்போது பேசுகிறது.",
  "chat.maestro.listening": "கேட்கிறேன்...",
  "chat.maestro.title.listening": "மேஸ்ட்ரோ உங்கள் உள்ளீடு அல்லது குரலுக்காக காத்திருக்கிறது.",
  "chat.maestro.holding": "மேஸ்ட்ரோ காத்திருக்கிறது",
  "chat.maestro.title.holding": "மேஸ்ட்ரோ காத்திருக்கிறது (மீண்டும் இணைவது இடைநிறுத்தப்பட்டது)",

  // Chat - bookmark
  "chat.bookmark.hiddenHeaderAria": "மேலே மறைக்கப்பட்ட செய்திகள்",
  "chat.bookmark.isHere": "புக்மார்க் இங்கே உள்ளது",
  "chat.bookmark.setHere": "இங்கே புக்மார்க் அமைக்கவும்",
  "chat.bookmark.actionsRegionAria": "புக்மார்க் செயல்கள்",
  "chat.bookmark.actionsToggleTitle": "புக்மார்க் விருப்பங்கள்",
  "chat.bookmark.decrementAria": "ஒன்று குறைவாகக் காட்டவும்",
  "chat.bookmark.decrementTitle": "குறைவு",
  "chat.bookmark.incrementAria": "ஒன்று அதிகமாகக் காட்டவும்",
  "chat.bookmark.incrementTitle": "அதிகம்",
  "chat.bookmark.hiddenBelowHeaderAria": "கீழே மறைக்கப்பட்ட செய்திகள்",

  // Chat - send preparation
  "chat.sendPrep.optimizingVideo": "வீடியோவை மேம்படுத்துகிறது...",
  "chat.sendPrep.optimizingImage": "படத்தை மேம்படுத்துகிறது...",
  "chat.sendPrep.preparingMedia": "மீடியாவைத் தயாரிக்கிறது...",
  "chat.sendPrep.uploadingMedia": "மீடியாவை பதிவேற்றுகிறது...",
  "chat.sendPrep.finalizing": "இறுதிப்படுத்துகிறது...",

  // Chat - header activity tokens
  "chat.header.annotating": "குறிப்பு சேர்க்கிறது",
  "chat.header.recordingAudio": "ஆடியோ பதிவு செய்கிறது",
  "chat.header.recordingVideo": "வீடியோ பதிவு செய்கிறது",
  "chat.header.savePopup": "சேமிக்கிறது...",
  "chat.header.loadPopup": "ஏற்றுகிறது...",
  "chat.header.maestroAvatar": "மேஸ்ட்ரோ அவதாரத்தை புதுப்பிக்கிறது",
  "chat.header.watchingVideo": "வீடியோ பார்க்கிறது",
  "chat.header.viewingAbove": "முந்தைய செய்திகளைப் பார்க்கிறது",
  "chat.header.liveSession": "நேரடி அமர்வு",

  // Chat - live session
  "chat.liveSession.stop": "நேரடியை நிறுத்தவும்",
  "chat.liveSession.retry": "நேரடியை மீண்டும் முயற்சிக்கவும்",
  "chat.liveSession.start": "நேரடியைத் தொடங்கவும்",
  "chat.liveSession.liveBadge": "நேரடி",
  "chat.liveSession.connecting": "இணைக்கிறது",
  "chat.liveSession.defaultLastMessage": "வணக்கம்! இன்று நான் உங்களுக்கு எப்படி உதவ முடியும்?",
  "chat.liveSession.defaultSuggestion1": "வணக்கம்",
  "chat.liveSession.defaultSuggestion2": "காலை வணக்கம்",
  "chat.liveSession.defaultSuggestion3": "நீங்கள் எப்படி இருக்கிறீர்கள்?",
  "chat.liveSession.activeIndicator": "நேரடி வீடியோ அமர்வு செயல்பாட்டில் உள்ளது, மேஸ்ட்ரோ உங்களை பார்க்கவும் கேட்கவும் முடியும்.",

  // Chat - errors
  "chat.error.sttError": "STT பிழை: {error}. மைக்கை மாற்றி முயற்சிக்கவும்.",
  "chat.error.autoCaptureCameraError": "தானியங்கி கேமரா பிடிப்பு பிழை: {error}",
  "chat.error.snapshotUserError": "{error}",
  "chat.error.recordingTimeExceeded": "{maxMinutes} நிமிடங்களுக்குப் பிறகு பதிவு தானாக நிறுத்தப்பட்டது.",
  "chat.error.videoMetadataError": "வீடியோ மெட்டாடேட்டாவைப் படிக்க முடியவில்லை. கோப்பு சிதைந்திருக்கலாம் அல்லது ஆதரிக்கப்படாத வடிவத்தில் இருக்கலாம்.",
  "chat.error.pauseVideoToAnnotate": "தற்போதைய ஃப்ரேமில் குறிப்பு சேர்க்க வீடியோவை இடைநிறுத்தவும்",
  "chat.error.imageGenInterrupted": "படம் உருவாக்கம் தடைப்பட்டது.",
  "chat.error.thinkingInterrupted": "AI-இன் பதில் தடைப்பட்டது.",

  // Errors - general
  "error.noLanguagePair": "முக்கியமான பிழை: மொழி ஜோடி எதுவும் தேர்ந்தெடுக்கப்படவில்லை.",
  "error.translationFailed": "மொழிபெயர்ப்பு தோல்வியடைந்தது. மீண்டும் முயற்சிக்கவும்.",
  "error.imageLimitReached": "அமர்வு படம் உருவாக்கும் வரம்பை அடைந்தது. புதிய அமர்வைத் தொடங்கவும்.",
  "error.tokenLimitReached": "அமர்வு டோக்கன் வரம்பை அடைந்தது. புதிய அமர்வைத் தொடங்கவும்.",
  "error.apiKeyMissing": "உங்கள் Gemini API சாவி இல்லை. API சாவி திரையைத் திறந்து உங்கள் சாவியை ஒட்டவும்.",
  "error.apiKeyInvalid": "உங்கள் Gemini API சாவி தவறானது. எழுத்துப் பிழைகளைச் சரிபார்த்து சரியான சாவியை ஒட்டவும்.",
  "error.apiQuotaExceeded": "உங்கள் ஒதுக்கீடு தீர்ந்துவிட்டது (கட்டணம்: சிறந்த அரட்டை, வீடியோ, பிடிஎஃப் மற்றும் பட உருவாக்கம்)",
  "error.quotaSetupBilling": "பில்லிங்கை அமை",
  "error.quotaStartLive": "நேரலை இலவச பதிப்பைத் தொடங்கு (பட உருவாக்கம் இல்லை)",
  "error.imageGenCostWarning": "பட உருவாக்கத்திற்கு ஒரு படத்திற்கு ~$0.04 செலவாகும். அரட்டை செய்திகளுக்கும் சிறிய செலவு உள்ளது. அனைத்தும் Google ஆல் பில் செய்யப்படுகின்றன. ஏபிஐ கீ திரையில் நீங்கள் செலவுகளைக் கண்காணிக்கலாம்.",
  "error.imageGenDisable": "பட உருவாக்கத்தை முடக்கு",
  "error.imageGenViewCost": "ஏபிஐ கீ திரையில் செலவுகளைக் கண்காணிக்கவும்.",
  // Errors - camera
  "error.cameraPermissionDenied": "கேமரா அனுமதி மறுக்கப்பட்டது. உலாவி அமைப்புகளில் கேமரா அணுகலை இயக்கவும்.",
  "error.cameraNotFound": "தேர்ந்தெடுக்கப்பட்ட கேமரா கண்டறியப்படவில்லை. இணைக்கப்பட்டுள்ளதா என்பதை உறுதிசெய்யவும் அல்லது வேறு கேமராவைத் தேர்ந்தெடுக்கவும்.",
  "error.cameraAccessNotSupported": "உங்கள் உலாவியால் கேமரா அணுகல் ஆதரிக்கப்படவில்லை.",
  "error.cameraUnknown": "கேமராவை அணுகும்போது தெரியாத பிழை ஏற்பட்டது.",
  "error.cameraStreamNotAvailable": "பிடிப்புக்கு கேமரா ஸ்ட்ரீம் கிடைக்கவில்லை.",
  "error.imageCaptureGeneric": "படம் பிடிக்கும்போது தெரியாத பிழை.",

  // Errors - visual context
  "error.visualContextVideoElementNotReady": "விஷுவல் கான்டெக்ஸ்ட் வீடியோ எலிமென்ட் தயாராக இல்லை.",
  "error.snapshotVideoElementNotReady": "ஸ்னாப்ஷாட் வீடியோ எலிமென்ட் தயாராக இல்லை.",
  "error.visualContextCameraAccessNotSupported": "விஷுவல் கான்டெக்ஸ்டிற்கு கேமரா அணுகல் ஆதரிக்கப்படவில்லை.",
  "error.snapshotCameraAccessNotSupported": "ஸ்னாப்ஷாட்டிற்கு கேமரா அணுகல் ஆதரிக்கப்படவில்லை.",
  "error.visualContext2DContext": "விஷுவல் கான்டெக்ஸ்டிற்கு 2D கான்டெக்ஸ்ட் பெற முடியவில்லை.",
  "error.snapshot2DContext": "ஸ்னாப்ஷாட்டிற்கு 2D கான்டெக்ஸ்ட் பெற முடியவில்லை.",
  "error.visualContextCaptureFailedPermission": "விஷுவல் கான்டெக்ஸ்ட் தோல்வி: கேமரா அனுமதி மறுக்கப்பட்டது.",
  "error.snapshotCaptureFailedPermission": "ஸ்னாப்ஷாட் தோல்வி: கேமரா அனுமதி மறுக்கப்பட்டது.",
  "error.visualContextCaptureFailedNotFound": "விஷுவல் கான்டெக்ஸ்ட் தோல்வி: கேமரா கண்டறியப்படவில்லை.",
  "error.snapshotCaptureFailedNotFound": "ஸ்னாப்ஷாட் தோல்வி: கேமரா கண்டறியப்படவில்லை.",
  "error.visualContextCaptureFailedNotReady": "விஷுவல் கான்டெக்ஸ்ட் தோல்வி: கேமரா தயாராக இல்லை அல்லது ஃபீடில் சிக்கல். {details}",
  "error.snapshotCaptureFailedNotReady": "ஸ்னாப்ஷாட் தோல்வி: கேமரா தயாராக இல்லை அல்லது ஃபீடில் சிக்கல். {details}",
  "error.visualContextCaptureFailedGeneric": "விஷுவல் கான்டெக்ஸ்ட் தோல்வி: {details}",
  "error.snapshotCaptureFailedGeneric": "ஸ்னாப்ஷாட் தோல்வி: {details}",

  // New UI strings
  "app.loading": "ஆப் ஏற்றுகிறது...",
  "audio.loading": "ஆடியோ ஏற்றுகிறது",
  "audio.pause": "இடைநிறுத்து",
  "audio.play": "இயக்கு",
  "audio.progress": "ஆடியோ நிலை",
  "camera.observerActive": "ஆய்வாளர் செயலில்",
  "chat.bookmark.cancel": "ரத்துசெய்",
  "chat.bookmark.editAria": "புக்மார்க் சுருக்கத்தைத் திருத்தவும்",
  "chat.bookmark.hiddenBelow": "செய்திகள் கீழே மறைக்கப்பட்டுள்ளன",
  "chat.bookmark.hiddenCount": "மேலே {count} செய்திகள் மறைக்கப்பட்டுள்ளன",
  "chat.bookmark.returnToRecent": "கீழே உள்ள செய்திகளுக்குத் திரும்பவும்",
  "chat.bookmark.save": "சேமி",
  "chat.bookmark.showAboveChunk": "மேலே உள்ள செய்திகளைக் காட்டு",
  "chat.bookmark.showNextChunk": "மேலே {count} காட்டு",
  "chat.bookmark.showPreviousChunk": "கீழே {count} காட்டு",
  "chat.bookmark.showingAboveRange": "{total} இல் {start}-{end} க்கு மேலே உள்ள செய்திகளைக் காட்டுகிறது",
  "chat.bookmark.summaryPlaceholder": "சுருக்கம்...",
  "chat.camera.capturePhoto": "புகைப்படம் எடு",
  "chat.camera.startRecording": "வீடியோ பதிவைத் தொடங்கவும்",
  "chat.deleteMessage": "செய்தியை நீக்கவும்",
  "chat.drawMessage": "செய்தியை வரையவும்",
  "chat.focusedMode.enter": "கவனத்திரட்டு பயன்முறையில் நுழைக",
  "chat.focusedMode.exit": "கவனத்திரட்டு பயன்முறையிலிருந்து வெளியேறு",
  "chat.music.finalizing": "இசையைச் சேமிக்கிறது...",
  "chat.music.starting": "இசையைத் தொடங்குகிறது...",
  "chat.music.streaming": "இசையை ஸ்ட்ரீம் செய்கிறது...",
  "chat.pdf.loadFailed": "PDF ஐ ஏற்றுவதில் தோல்வி",
  "debugLog.clearLogs": "பதிவுகளை அழி",
  "debugLog.clearUploads": "தற்போதைய API சாவிக்கான அனைத்து Gemini பதிவேற்றங்களையும் நீக்கவும்",
  "debugLog.close": "மூடு",
  "debugLog.error": "பிழை",
  "debugLog.noTraffic": "இன்னும் போக்குவரத்து பதிவு செய்யப்படவில்லை.",
  "debugLog.requestPayload": "கோரிக்கை சேர்வரிவு",
  "debugLog.response": "பதில்",
  "debugLog.title": "போக்குவரத்து பதிவு",
  "error.cameraOverconstrained": "கோரிய அமைப்புகளுடன் கேமரா கிடைக்கவில்லை. {errorMessage}",
  "globalProfile.ariaLabel": "உலகளாவிய சுயவிவர சுருக்கம்",
  "globalProfile.loading": "சுயவிவரம் ஏற்றுகிறது...",
  "globalProfile.noProfile": "இன்னும் சுயவிவரம் இல்லை.",
  "header.apiKey": "API சாவி",
  "header.apiKeyRequired": "API சாவி தேவை",
  "header.clickToViewStatus": "நிலையைப் பார்க்க கிளிக் செய்யவும்",
  "header.manageApiKey": "API சாவியை நிர்வகிக்கவும்",
  "header.pauseMaestro": "Maestro ஐ இடைநிறுத்தவும்",
  "header.resumeMaestro": "Maestro ஐ மீண்டும் தொடங்கவும்",
  "header.viewTrafficLogs": "போக்குவரத்து பதிவுகளைப் பார்க்கவும்",
  "miniGame.codeEmpty": "மினி கேம் குறியீடு காலியாக உள்ளது.",
  "miniGame.failedToRender": "மினி கேம் வழங்குவதில் தோல்வி.",
  "miniGame.fullscreen": "முழுத்திரை",
  "miniGame.hideCode": "குறியீட்டை மறை",
  "miniGame.launching": "மினி கேம் தொடங்குகிறது...",
  "miniGame.restart": "மீண்டும் தொடங்கு",
  "miniGame.retry": "மீண்டும் முயற்சி",
  "miniGame.runtimeError": "மினி கேம் பிழை: {error}",
  "miniGame.showCode": "குறியீட்டைக் காட்டு",
  "miniGame.title": "மினி கேம்",
  "miniGame.titleWithFile": "மினி கேம் {fileName}",
  "officeFile.failedToParse": "உள்ளடக்கிய முன்னோட்டத்தை பாகுபடுத்துவதில் தோல்வி.",
  "officeFile.localPreviewUnavailable": "உள்ளூர் முன்னோட்டம் கிடைக்கவில்லை. உள்ளூரில் திறக்க மீண்டும் இணைக்கவும்.",
  "officeFile.openFile": "கோப்பைத் திற",
  "officeFile.openInGoogleWorkspace": "Google Workspace இல் திற",
  "officeFile.parsingInlinePreview": "உள்ளடக்கிய முன்னோட்டத்தை பாகுபடுத்துகிறது...",
  "officeFile.parsingPreview": "முன்னோட்டத்தை பாகுபடுத்துகிறது...",
  "officeFile.previewUnavailable": "இந்த கோப்பிற்கு முன்னோட்டம் கிடைக்கவில்லை.",
  "officeFile.rawExtractedText": "மூல பிரித்தெடுக்கப்பட்ட உரை",
  "sessionControls.customizeColors": "நிறங்களை தனிப்பயனாக்கவும்",
  "streaming.connectingTo": "{model} உடன் இணைக்கிறது (முயற்சி {attempt}/{total})...",
  "streaming.failedToPrepareAttachment": "அனுப்புவதற்கு சமீபத்திய இணைப்பு \"{name}\" தயாரிப்பதில் தோல்வி. மீண்டும் முயற்சிக்கவும் அல்லது கோப்பை மீண்டும் இணைக்கவும்.",
  "streaming.failedToPrepareCurrent": "இணைக்கப்பட்ட மீடியா \"{name}\" ஐ Gemini க்காக தயாரிப்பதில் தோல்வி. மீண்டும் முயற்சிக்கவும் அல்லது கோப்பை மீண்டும் இணைக்கவும்.",
  "streaming.highDemand": "அதிக தேவை கண்டறியப்பட்டது. கோரிக்கை Google சேவையகங்களில் வரிசையில் உள்ளது.",
  "streaming.noOutputAfter": "{seconds}s க்கு பிறகு மாதிரி வெளியீடு இல்லை. கோரிக்கை Google சேவையகங்களில் வரிசையில் இருக்கலாம்.",
  "streaming.phaseConnecting": "இணைக்கிறது",
  "streaming.phaseFinalResponse": "இறுதி பதில்",
  "streaming.phaseFinalizing": "இறுதிப்படுத்துகிறது",
  "streaming.phaseHighDemand": "அதிக தேவை",
  "streaming.phasePreparingRequest": "கோரிக்கை தயாரிக்கிறது",
  "streaming.phaseProcessing": "செயலாக்குகிறது",
  "streaming.phaseRetrying": "மீண்டும் முயற்சிக்கிறது",
  "streaming.phaseSwitchingModel": "மாதிரி மாற்றுகிறது",
  "streaming.phaseThinking": "யோசிக்கிறது",
  "streaming.preparingRequestContext": "கோரிக்கை சூழலை தயாரிக்கிறது...",
  "streaming.retryingIn": "{seconds}s இல் மீண்டும் முயற்சிக்கிறது...",
  "streaming.switchingFallback": "வழிமாற்று மாதிரி {model} க்கு மாறுகிறது.",
  "streaming.waitingForOutput": "மாதிரி வெளியீட்டிற்காக காத்திருக்கிறது... {seconds}s கடந்தது.",
  "textFile.rawText": "மூல உரை",
  "textFile.unableToDecode": "உரை இணைப்பை டிகோட் செய்ய இயலவில்லை.",
  "themeCustomizer.cancel": "ரத்துசெய்",
  "themeCustomizer.confirm": "உறுதிப்படுத்து",
  "themeCustomizer.deletePreset": "சேமித்த முன்னமைவை நீக்கவும்",
  "themeCustomizer.exportAs": "என்று ஏற்றுமதி செய்யவும்",
  "themeCustomizer.exportToFile": "கோப்பிற்கு ஏற்றுமதி செய்யவும்",
  "themeCustomizer.importFromFile": "கோப்பிலிருந்து இறக்குமதி செய்யவும்",
  "themeCustomizer.instruction": "ஒரு வண்ண வட்டத்தை தொட்டு, பின் தேர்வியில் இழுக்கவும். மாற்றங்கள் உடனடியாக பயன்படுத்தப்படும்.",
  "themeCustomizer.namePlaceholder": "கருப்பொருள் பெயர்...",
  "themeCustomizer.quickThemes": "விரைவு கருப்பொருள்கள்",
  "themeCustomizer.resetAll": "அனைத்தையும் மீட்டமை",
  "themeCustomizer.saveAs": "என்று சேமிக்கவும்",
  "themeCustomizer.saveAsPreset": "முன்னமைவாகச் சேமிக்கவும்",
  "themeCustomizer.storeDescription": "மேலும் கருப்பொருள்களை உலாவி வாங்கவும்",
  "themeCustomizer.storeTitle": "கருப்பொருள் கடை",
  "themeCustomizer.title": "வண்ணங்களை வரையவும்",
  "themeStore.buy": "வாங்கு",
  "themeStore.close": "மூடு",
  "themeStore.footerNote": "கொள்முதல்கள் Google Play ஆல் நிர்வகிக்கப்படுகின்றன மற்றும் உங்கள் Google கணக்குடன் இணைக்கப்படுகின்றன. மறுநிறுவலில் முந்தைய கொள்முதல்களை மீட்டெடுக்க மீட்டமை பொத்தானை (↩) தொடவும்.",
  "themeStore.loading": "கருப்பொருள்களை ஏற்றுகிறது…",
  "themeStore.notAvailable": "கருப்பொருள் கொள்முதல்கள் Google Play வழியாக Android இல் மட்டுமே கிடைக்கும்.",
  "themeStore.owned": "சொந்தமானது",
  "themeStore.purchaseFailed": "கொள்முதல் தோல்வியடைந்தது. உங்கள் இணைப்பைச் சரிபார்த்து மீண்டும் முயற்சிக்கவும்.",
  "themeStore.restorePurchases": "கொள்முதல்களை மீட்டெடு",
  "themeStore.title": "கருப்பொருள் கடை",
};