// Copyright 2025 Roni Tervo
//
// SPDX-License-Identifier: Apache-2.0
export const teTranslations: Record<string, string> = {
  // App title
  "app.title": "Maestro",
  // Language selector (used in SttLanguageSelector)
  "sttLang.selectLanguage": "వాయిస్ రికగ్నిషన్‌ను {language}కి సెట్ చేయండి",

  // Header
  "header.targetLanguageTitle": "ప్రస్తుత లక్ష్య భాష: {language}",

  // Start page (used)
  "startPage.clickToStart": "విమానాన్ని క్లిక్ చేయండి",
  "startPage.saveChats": "అన్ని చాట్‌లను సేవ్ చేయండి",
  "startPage.loadChats": "చాట్‌లను లోడ్ చేయండి",
  "startPage.saveThisChat": "ఈ చాట్‌ను సేవ్ చేయండి",
  "startPage.appendToChat": "చాట్‌కు జోడించండి",
  "startPage.trimBeforeBookmark": "బుక్‌మార్క్‌కు ముందు ఉన్నవి తొలగించండి",
  "startPage.maestroAvatar": "మ్యాస్ట్రో అవతార్",
  "startPage.addMaestroAvatar": "మ్యాస్ట్రో అవతార్‌ను జోడించండి",
  "startPage.loadSuccess": "{count} చాట్ సెషన్‌లు విజయవంతంగా లోడ్ చేయబడ్డాయి మరియు భర్తీ చేయబడ్డాయి!",
  "startPage.loadError": "చాట్‌లను లోడ్ చేయడంలో లోపం. ఫైల్ పాడై ఉండవచ్చు లేదా తప్పు ఫార్మాట్‌లో ఉండవచ్చు.",
  "startPage.noChatsToSave": "సేవ్ చేయడానికి చాట్ చరిత్ర లేదు.",
  "startPage.saveError": "చాట్‌లను సేవ్ చేయడంలో లోపం. మరిన్ని వివరాల కోసం కన్సోల్‌ను చూడండి.",
  "startPage.noChatSelected": "దయచేసి ముందుగా భాషా జంటను ఎంచుకోండి.",
  "startPage.noBookmarkSet": "బుక్‌మార్క్ సెట్ చేయలేదు. సందేశాలను తొలగించడానికి ముందుగా బుక్‌మార్క్‌ను సెట్ చేయండి.",
  "startPage.noMessagesToTrim": "తొలగించడానికి బుక్‌మార్క్ కంటే ముందు సందేశాలు ఏవీ లేవు.",
  "startPage.trimSuccess": "బుక్‌మార్క్ కంటే ముందు ఉన్న {count} సందేశాలు విజయవంతంగా తొలగించబడ్డాయి.",
  "startPage.trimError": "సందేశాలను తొలగించడం విఫలమైంది. దయచేసి మళ్ళీ ప్రయత్నించండి.",
  "startPage.noMessagesToAppend": "బ్యాకప్ ఫైల్‌లో కలపడానికి సందేశాలు ఏవీ లేవు.",
  "startPage.noPairInBackup": "ఈ బ్యాకప్ ఫైల్‌లో మీ ప్రస్తుత భాషా జంటకు సంబంధించిన సందేశాలు లేవు. దయచేసి సరిపోయే బ్యాకప్‌ను ఎంచుకోండి.",
  "startPage.appendSuccess": "ప్రస్తుత చాట్‌కు {count} సందేశాలు విజయవంతంగా జోడించబడ్డాయి.",
  "startPage.combineSuccess": "చాట్‌లు కలిపబడ్డాయి: {added} కొత్త సందేశాలు జోడించబడ్డాయి, మొత్తం {total} సందేశాలు.",
  "startPage.combineNoDuplicates": "అన్ని సందేశాలు ఇప్పటికే మీ చాట్‌లో ఉన్నాయి. ఎటువంటి మార్పులు చేయలేదు.",
  "startPage.combineNoChanges": "జోడించడానికి కొత్త సందేశాలు ఏవీ లేవు. మీ చాట్ మారలేదు.",
  "startPage.invalidBackupFormat": "చెల్లని బ్యాకప్ ఫైల్. దయచేసి సరైన Maestro బ్యాకప్ (.ndjson) ఫైల్‌ను ఎంచుకోండి.",
  "startPage.browserNotSupported": "మీ బ్రౌజర్ ఫైల్ సేవింగ్‌కు మద్దతు ఇవ్వదు. దయచేసి Chrome లేదా Edge ఉపయోగించండి.",

  // Session Controls - Action labels and descriptions
  "sessionControls.saveAll.label": "అన్నీ సేవ్ చేయండి",
  "sessionControls.saveAll.description": "అన్ని చాట్‌లను బ్యాకప్ ఫైల్‌గా ఎగుమతి చేయండి",
  "sessionControls.loadAll.label": "అన్నీ లోడ్ చేయండి",
  "sessionControls.loadAll.description": "అన్ని చాట్‌లను బ్యాకప్ ఫైల్‌తో భర్తీ చేయండి",
  "sessionControls.reset.label": "రీసెట్ (Reset)",
  "sessionControls.reset.description": "డేటా అంతా బ్యాకప్ చేసి తొలగించండి",
  "sessionControls.saveThis.label": "చాట్‌ను సేవ్ చేయండి",
  "sessionControls.saveThis.description": "ఈ చాట్‌ను మాత్రమే ఎగుమతి చేయండి",
  "sessionControls.combine.label": "కలపండి",
  "sessionControls.combine.description": "బ్యాకప్‌ను ఈ చాట్‌లోకి విలీనం చేయండి",
  "sessionControls.trim.label": "తొలగించండి (Trim)",
  "sessionControls.trim.description": "బుక్‌మార్క్ కంటే ముందు సందేశాలను తొలగించండి",

  // Session Controls - UI elements
  "sessionControls.profile": "ప్రొఫైల్:",
  "sessionControls.profilePlaceholder": "మీ పేరు లేదా వివరాలు...",
  "sessionControls.editProfile": "యూజర్ ప్రొఫైల్‌ను సవరించండి",
  "sessionControls.allChatsControls": "అన్ని చాట్‌ల నియంత్రణలు",
  "sessionControls.thisChatsControls": "ఈ చాట్ నియంత్రణలు",
  "sessionControls.back": "వెనుకకు",
  "sessionControls.backupAndReset": "బ్యాకప్ మరియు రీసెట్",
  "sessionControls.typeToConfirm": "ధృవీకరించడానికి \"{keyword}\" అని టైప్ చేయండి",
  "sessionControls.changeAvatar": "అవతార్ మార్చండి",

  // General
  "general.clear": "క్లియర్ చేయండి",
  "general.error": "క్షమించండి, ఒక లోపం సంభవించింది.",

  // API key gate
  "apiKeyGate.title": "మీ Gemini API కీని కనెక్ట్ చేయండి",
  "apiKeyGate.billingTitle": "అధిక కోటా కోసం బిల్లింగ్‌ను సెటప్ చేయండి",
  "apiKeyGate.infoLogin": "మీ లాగిన్: ఒక API కీ",
  "apiKeyGate.infoVisibility": "ఎవరు చూస్తారు: మీరు మాత్రమే",
  "apiKeyGate.infoBilling": "బిల్లింగ్: నెలవారీ, Google నుండి",
  "apiKeyGate.infoCost": "ఖర్చు: యాప్‌లో చూపబడుతుంది — చూడటానికి నొక్కండి",
  "apiKeyGate.infoMore": "మరింత సమాచారం:",
  "apiKeyGate.privacyPolicy": "గోప్యతా విధానం",
  "apiKeyGate.stepsTitle": "రెండు శీఘ్ర దశలు:",
  "apiKeyGate.stepOne": "Google AI Studioని తెరిచి API కీని సృష్టించండి.",
  "apiKeyGate.stepTwo": "క్రింద కీని అతికించి, సేవ్ (Save) చేయండి.",
  "apiKeyGate.openAiStudio": "Google AI Studioని తెరవండి",
  "apiKeyGate.viewInstructions": "సూచనలను చూడండి",
  "apiKeyGate.closeInstructions": "సూచనలను ముగించండి",
  "apiKeyGate.previousInstruction": "మునుపటి సూచన",
  "apiKeyGate.nextInstruction": "తర్వాతి సూచన",
  "apiKeyGate.instructionStep": "సూచన {total}లో {step}",
  "apiKeyGate.keyLabel": "Gemini API కీ",
  "apiKeyGate.placeholder": "మీ API కీని ఇక్కడ అతికించండి",
  "apiKeyGate.show": "చూపించు",
  "apiKeyGate.hide": "దాచు",
  "apiKeyGate.currentKeySaved": "ప్రస్తుత కీ సేవ్ చేయబడింది {maskedKey}",
  "apiKeyGate.keyInvalid": "కీ చెల్లదు {maskedKey} — క్రింద కొత్త కీని పేస్ట్ చేయండి",
  "apiKeyGate.clearSavedKey": "సేవ్ చేసిన కీని క్లియర్ చేయండి",
  "apiKeyGate.cancel": "రద్దు చేయండి",
  "apiKeyGate.saving": "సేవ్ అవుతోంది...",
  "apiKeyGate.saveKey": "కీని సేవ్ చేయండి",
  "apiKeyGate.close": "ముగించు",
  "apiKeyGate.costLabel": "అంచనా API ఖర్చు",
  // API key gate - tester form
  "apiKeyGate.testerFormTitle": "యాప్‌ని టెస్ట్ చేయండి",
  "apiKeyGate.testerFormDescription": "మా Android బీటా ఆహ్వానాన్ని పొందడానికి మీరు Google Play Store కోసం ఉపయోగించే ఇమెయిల్‌ను నమోదు చేయండి!",
  "apiKeyGate.testerFormSubmit": "ఎర్లీ యాక్సెస్ పొందండి",
  "apiKeyGate.testerFormSubmitting": "పంపుతోంది...",
  "apiKeyGate.testerFormError": "ఏదో తప్పు జరిగింది. దయచేసి మళ్లీ ప్రయత్నించండి.",
  "apiKeyGate.testerFormMustBeGmail": "Google Play కోసం దయచేసి @gmail.com అడ్రస్‌ని ఉపయోగించండి.",
  "apiKeyGate.testerFormCapReached": "సబ్మిషన్ పరిమితి పూర్తయింది. ధన్యవాదాలు!",
  "apiKeyGate.developerLogin": "డెవలపర్ లాగిన్",
  "apiKeyGate.submitAnotherEmail": "తప్పు ఇమెయిల్ ఇచ్చారా? రీసెట్ చేసి మరొకటి ప్రయత్నించండి.",
  "apiKeyGate.testerFormChecking": "మీ ఆహ్వాన స్థితిని తనిఖీ చేస్తోంది...",
  "apiKeyGate.testerFormApprovedTitle": "మీరు చేరారు! 🎉",
  "apiKeyGate.testerFormApprovedDesc": "Android బీటా టెస్టింగ్ ప్రోగ్రామ్ కోసం మీ ఇమెయిల్ ఆమోదించబడింది.",
  "apiKeyGate.testerFormDownloadBtn": "Google Playలో యాప్‌ని డౌన్‌లోడ్ చేయండి",
  "apiKeyGate.testerFormPendingTitle": "మీరు వెయిట్‌లిస్ట్‌లో ఉన్నారు!",
  "apiKeyGate.testerFormPendingDesc": "మాకు మీ ఇమెయిల్ అందింది. ఆహ్వానాన్ని ప్రాసెస్ చేయడానికి కొంత సమయం పడుతుంది. దయచేసి తర్వాత తనిఖీ చేయండి!",
  "apiKeyGate.checkStatusModeBtn": "ఇప్పటికే అప్లై చేశారా? మీ స్థితిని తనిఖీ చేయండి.",
  "apiKeyGate.submitModeBtn": "అప్లై చేయాలా? ఇక్కడ సైన్ అప్ చేయండి.",
  "apiKeyGate.checkStatusTitle": "ఆహ్వాన స్థితిని తనిఖీ చేయండి",
  "apiKeyGate.checkStatusDesc": "వెయిట్‌లిస్ట్ కోసం మీరు ఉపయోగించిన ఇమెయిల్‌ను నమోదు చేయండి.",
  "apiKeyGate.checkStatusSubmit": "స్థితిని తనిఖీ చేయండి",
  "apiKeyGate.testerFormNotFound": "వెయిట్‌లిస్ట్‌లో ఇమెయిల్ కనుగొనబడలేదు. దయచేసి మొదట సైన్ అప్ చేయండి.",
  // Chat - general
  "chat.thinking": "ఆలోచిస్తోంది...",
  "chat.loadingHistory": "చాట్ చరిత్రను లోడ్ చేస్తోంది...",
  "chat.loadingSuggestions": "సూచనలను లోడ్ చేస్తోంది...",
  "chat.suggestionsAriaLabel": "సమాధాన సూచనలు",
  "chat.attachImageFromFile": "ఫైల్‌ను జోడించండి",
  "chat.removeAttachedImage": "జోడించిన ఫైల్‌ను తొలగించండి",
  "chat.sendMessage": "సందేశాన్ని పంపండి",
  "chat.messageInputAriaLabel": "సందేశ ఇన్‌పుట్",
  "chat.retrievedFromWeb": "వెబ్ నుండి పొందబడింది:",
  "chat.videoNotSupported": "మీ బ్రౌజర్ వీడియో ట్యాగ్‌కు మద్దతు ఇవ్వదు.",
  "chat.audioNotSupported": "మీ బ్రౌజర్ ఆడియో ట్యాగ్‌కు మద్దతు ఇవ్వదు.",
  "chat.fileAttachment": "ఫైల్ జోడింపు",
  "chat.pdf.loading": "PDF లోడ్ అవుతోంది...",
  "chat.pdf.error": "PDF ప్రదర్శించడం సాధ్యం కాలేదు",
  "chat.pdf.pageIndicator": "పేజీ {current} / {total}",
  "chat.imageGenError": "చిత్రం తయారీ లోపం",
  "chat.generatingImageLoadingSlow": "కొంచెం ఎక్కువ సమయం పడుతోంది...",
  "chat.stopSpeaking": "మాట్లాడటం ఆపండి",
  "chat.speakThisLine": "ఈ లైన్‌ను మాట్లాడండి",
  "chat.languageSelector.openGlobe": "భాషలను మార్చండి",
  "chat.maestroTranscriptScrollwheel": "మ్యాస్ట్రో ట్రాన్స్‌క్రిప్ట్ స్క్రోల్ వీక్షణ",

  // Chat - mic/STT
  "chat.mic.listening": "STT యాక్టివ్: వింటోంది...",
  "chat.mic.enableStt": "STTని ఆన్ చేయండి",
  "chat.mic.disableStt": "STTని ఆపండి",
  "chat.mic.recordingAudioNote": "ఆడియో రికార్డ్ అవుతోంది...",

  // Chat - placeholders
  "chat.placeholder.normal.listening": "{language}లో వింటోంది...",
  "chat.placeholder.normal.sttActive": "{language}లో మాట్లాడండి లేదా టైప్ చేయండి...",
  "chat.placeholder.normal.sttInactive": "టైప్ చేయండి లేదా {language}లో మాట్లాడటానికి మైక్‌ను నొక్కండి...",
  "chat.placeholder.suggestion.listening": "అనువదించడానికి {language} మాట్లాడండి...",
  "chat.placeholder.suggestion.sttActive": "అనువదించడానికి {language}లో మాట్లాడండి లేదా టైప్ చేయండి...",
  "chat.placeholder.suggestion.sttInactive": "అనువదించడానికి {language}లో టైప్ చేయండి...",

  // Chat - camera
  "chat.camera.turnOn": "కెమెరా ప్రివ్యూని ఆన్ చేయండి",
  "chat.camera.turnOff": "కెమెరా ప్రివ్యూని ఆఫ్ చేయండి",
  "chat.camera.selectCamera": "కెమెరాను ఎంచుకోండి",
  "chat.camera.imageGenCameraLabel": "చిత్రం తయారీ",
  "chat.camera.captureOrRecord": "ఫోటో కోసం నొక్కండి, వీడియో కోసం హోల్డ్ చేయండి",
  "chat.camera.stopRecording": "రికార్డింగ్‌ను ఆపండి",
  "chat.bookIcon.toggleImageGen": "చిత్ర తయారీ మోడ్‌ను మార్చండి",

  // Chat - image
  "chat.imagePreview.alt": "ప్రివ్యూ",
  "chat.image.dragToEnlarge": "పెద్దది చేయడానికి మూలని లాగండి",
  "chat.image.dragToShrink": "చిన్నది చేయడానికి మూలని లాగండి",
  "chat.annotateImage": "చిత్రంపై వ్యాఖ్యానించండి",
  "chat.annotateVideoFrame": "ప్రస్తుత ఫ్రేమ్‌పై వ్యాఖ్యానించండి",

  // Chat - annotate modal
  "chat.annotateModal.editingPreviewAlt": "వ్యాఖ్యానించాల్సిన చిత్రం",
  "chat.annotateModal.cancel": "రద్దు చేయండి",
  "chat.annotateModal.saveAndAttach": "సేవ్ చేయండి మరియు జోడించండి",
  "chat.annotateModal.undo": "అన్డు",

  // Chat - suggestions
  "chat.suggestion.speak": "చెప్పండి: \"{suggestion}\"",
  "chat.suggestion.ariaLabel": "సూచనను మాట్లాడండి: {suggestion}",
  "chat.suggestion.toggleCreateMode": "సూచన తయారీ మోడ్‌ను మార్చండి",
  "chat.suggestion.createAction": "సూచనను సృష్టించండి",
  "chat.suggestion.creating": "సూచనను సృష్టిస్తోంది...",

  // Chat - maestro status
  "chat.maestro.idle": "మ్యాస్ట్రో పనిలేకుండా ఉంది",
  "chat.maestro.title.idle": "మ్యాస్ట్రో ప్రస్తుతం పనిలేకుండా ఉంది.",
  "chat.maestro.resting": "మ్యాస్ట్రో విశ్రాంతి తీసుకుంటోంది...",
  "chat.maestro.observing": "మ్యాస్ట్రో గమనిస్తోంది...",
  "chat.maestro.aboutToEngage": "మ్యాస్ట్రో త్వరలో చేరనుంది...",
  "chat.maestro.title.resting": "మ్యాస్ట్రో పనిలేకుండా ఉంది, తిరిగి యాక్టివ్ అవ్వడానికి చాలా సమయం ఉంది.",
  "chat.maestro.title.observing": "మ్యాస్ట్రో గమనిస్తోంది, తిరిగి యాక్టివ్ అవ్వడానికి కొంచెం సమయం ఉంది.",
  "chat.maestro.title.aboutToEngage": "మ్యాస్ట్రో త్వరలో తిరిగి యాక్టివ్ అవుతుంది.",
  "chat.maestro.typing": "మ్యాస్ట్రో టైప్ చేస్తోంది...",
  "chat.maestro.title.typing": "మ్యాస్ట్రో సమాధానాన్ని సిద్ధం చేస్తోంది.",
  "chat.maestro.speaking": "మ్యాస్ట్రో మాట్లాడుతోంది",
  "chat.maestro.title.speaking": "మ్యాస్ట్రో ఇప్పుడు మాట్లాడుతోంది.",
  "chat.maestro.listening": "వింటోంది...",
  "chat.maestro.title.listening": "మ్యాస్ట్రో మీ ఇన్‌పుట్ లేదా వాయిస్ కోసం వేచి ఉంది.",
  "chat.maestro.holding": "మ్యాస్ట్రో వేచి ఉంది",
  "chat.maestro.title.holding": "మ్యాస్ట్రో వేచి ఉంది (తిరిగి యాక్టివ్ అవ్వడం పాజ్ చేయబడింది)",

  // Chat - bookmark (used)
  "chat.bookmark.hiddenHeaderAria": "పైన దాచిన సందేశాలు",
  "chat.bookmark.isHere": "బుక్‌మార్క్ ఇక్కడ ఉంది",
  "chat.bookmark.setHere": "ఇక్కడ బుక్‌మార్క్ సెట్ చేయండి",
  "chat.bookmark.actionsRegionAria": "బుక్‌మార్క్ చర్యలు",
  "chat.bookmark.actionsToggleTitle": "బుక్‌మార్క్ ఎంపికలు",
  "chat.bookmark.decrementAria": "ఒకటి తక్కువ చూపించు",
  "chat.bookmark.decrementTitle": "తక్కువ",
  "chat.bookmark.incrementAria": "ఒకటి ఎక్కువ చూపించు",
  "chat.bookmark.incrementTitle": "ఎక్కువ",
  "chat.bookmark.hiddenBelowHeaderAria": "క్రింద దాచిన సందేశాలు",

  // Chat - send preparation
  "chat.sendPrep.optimizingVideo": "వీడియోను ఆప్టిమైజ్ చేస్తోంది...",
  "chat.sendPrep.optimizingImage": "చిత్రాన్ని ఆప్టిమైజ్ చేస్తోంది...",
  "chat.sendPrep.preparingMedia": "మీడియాను సిద్ధం చేస్తోంది...",
  "chat.sendPrep.uploadingMedia": "మీడియాను అప్‌లోడ్ చేస్తోంది...",
  "chat.sendPrep.finalizing": "పూర్తి చేస్తోంది...",

  // Chat - header activity tokens (used via activityTokens.ts)
  "chat.header.annotating": "వ్యాఖ్యానిస్తోంది",
  "chat.header.recordingAudio": "ఆడియో రికార్డ్ అవుతోంది",
  "chat.header.recordingVideo": "వీడియో రికార్డ్ అవుతోంది",
  "chat.header.savePopup": "సేవ్ అవుతోంది...",
  "chat.header.loadPopup": "లోడ్ అవుతోంది...",
  "chat.header.maestroAvatar": "మ్యాస్ట్రో అవతార్‌ను అప్‌డేట్ చేస్తోంది",
  "chat.header.watchingVideo": "వీడియో చూస్తోంది",
  "chat.header.viewingAbove": "మునుపటి సందేశాలను చూస్తోంది",
  "chat.header.liveSession": "లైవ్ సెషన్",

  // Chat - live session
  "chat.liveSession.stop": "లైవ్‌ని ఆపండి",
  "chat.liveSession.retry": "లైవ్‌ని మళ్ళీ ప్రయత్నించండి",
  "chat.liveSession.start": "లైవ్‌ని ప్రారంభించండి",
  "chat.liveSession.liveBadge": "లైవ్",
  "chat.liveSession.connecting": "కనెక్ట్ అవుతోంది",
  "chat.liveSession.defaultLastMessage": "నమస్కారం! ఈరోజు నేను మీకు ఎలా సహాయపడగలను?",
  "chat.liveSession.defaultSuggestion1": "నమస్కారం",
  "chat.liveSession.defaultSuggestion2": "శుభోదయం",
  "chat.liveSession.defaultSuggestion3": "మీరు ఎలా ఉన్నారు?",
  "chat.liveSession.activeIndicator": "లైవ్ వీడియో కాల్ సక్రియంగా ఉంది, మాస్ట్రో మీను చూడగలడు మరియు వినగలడు.",

  // Chat - errors
  "chat.error.sttError": "STT లోపం: {error}. మైక్‌ని మార్చడానికి ప్రయత్నించండి.",
  "chat.error.autoCaptureCameraError": "ఆటో క్యాప్చర్ కెమెరా లోపం: {error}",
  "chat.error.snapshotUserError": "{error}",
  "chat.error.recordingTimeExceeded": "{maxMinutes} నిమిషాల తర్వాత రికార్డింగ్ ఆటోమేటిక్‌గా ఆగిపోయింది.",
  "chat.error.videoMetadataError": "వీడియో మెటాడేటాను చదవలేకపోయింది. ఫైల్ పాడై ఉండవచ్చు లేదా మద్దతు లేని ఫార్మాట్ కావచ్చు.",
  "chat.error.pauseVideoToAnnotate": "ప్రస్తుత ఫ్రేమ్‌పై వ్యాఖ్యానించడానికి వీడియోను పాజ్ చేయండి",
  "chat.error.imageGenInterrupted": "చిత్ర తయారీకి అంతరాయం కలిగింది.",
  "chat.error.thinkingInterrupted": "AI ప్రతిస్పందనకు అంతరాయం కలిగింది.",

  // Errors - general
  "error.noLanguagePair": "కీలక లోపం: ఏ భాషా జంట ఎంచుకోబడలేదు.",
  "error.translationFailed": "అనువాదం విఫలమైంది. దయచేసి మళ్ళీ ప్రయత్నించండి.",
  "error.imageLimitReached": "సెషన్ చిత్ర పరిమితి చేరుకుంది. దయచేసి కొత్త సెషన్ ప్రారంభించండి.",
  "error.tokenLimitReached": "సెషన్ టోకెన్ పరిమితి చేరుకుంది. దయచేసి కొత్త సెషన్ ప్రారంభించండి.",
  "error.apiKeyMissing": "మీ Gemini API కీ లేదు. API కీ స్క్రీన్‌ను తెరిచి మీ కీని అతికించండి.",
  "error.apiKeyInvalid": "మీ Gemini API కీ చెల్లదు. దయచేసి టైపోల కోసం తనిఖీ చేసి, చెల్లుబాటు అయ్యే కీని అతికించండి.",
  "error.apiQuotaExceeded": "మీ కోటా ముగిసింది (చెల్లింపు: స్మార్ట్ చాట్, వీడియో, పిడిఎఫ్ మరియు ఇమేజ్ జనరేషన్)",
  "error.quotaSetupBilling": "బిల్లింగ్ సెటప్ చేయండి",
  "error.quotaStartLive": "లైవ్ ఉచిత వెర్షన్‌ను ప్రారంభించండి (ఇమేజ్ జనరేషన్ లేదు)",
  "error.imageGenCostWarning": "ఇమేజ్ జనరేషన్‌కు ప్రతి ఇమేజ్‌కు సుమారు $0.04 ఖర్చవుతుంది. చాట్ సందేశాలకు కూడా చిన్న ఖర్చు ఉంటుంది. అన్నీ Google ద్వారా బిల్ చేయబడతాయి. మీరు API కీ స్క్రీన్‌లో ఖర్చులను ట్రాక్ చేయవచ్చు.",
  "error.imageGenDisable": "ఇమేజ్ జనరేషన్‌ను నిలిపివేయండి",
  "error.imageGenViewCost": "API కీ స్క్రీన్‌లో ఖర్చులను ట్రాక్ చేయండి.",
  // Errors - camera
  "error.cameraPermissionDenied": "కెమెరా అనుమతి నిరాకరించబడింది. బ్రౌజర్ సెట్టింగ్‌లలో కెమెరా యాక్సెస్‌ని ఎనేబుల్ చేయండి.",
  "error.cameraNotFound": "ఎంచుకున్న కెమెరా కనుగొనబడలేదు. ఇది కనెక్ట్ అయిందని నిర్ధారించుకోండి లేదా వేరే కెమెరాను ఎంచుకోండి.",
  "error.cameraAccessNotSupported": "మీ బ్రౌజర్ ద్వారా కెమెరా యాక్సెస్ మద్దతు లేదు.",
  "error.cameraUnknown": "కెమెరాను యాక్సెస్ చేస్తున్నప్పుడు తెలియని లోపం సంభవించింది.",
  "error.cameraStreamNotAvailable": "క్యాప్చర్ కోసం కెమెరా స్ట్రీమ్ అందుబాటులో లేదు.",
  "error.imageCaptureGeneric": "చిత్రాన్ని క్యాప్చర్ చేస్తున్నప్పుడు తెలియని లోపం.",

  // Errors - visual context
  "error.visualContextVideoElementNotReady": "విజువల్ కాంటెక్స్ట్ వీడియో ఎలిమెంట్ సిద్ధంగా లేదు.",
  "error.snapshotVideoElementNotReady": "స్నాప్‌షాట్ వీడియో ఎలిమెంట్ సిద్ధంగా లేదు.",
  "error.visualContextCameraAccessNotSupported": "విజువల్ కాంటెక్స్ట్ కోసం కెమెరా యాక్సెస్ మద్దతు లేదు.",
  "error.snapshotCameraAccessNotSupported": "స్నాప్‌షాట్ కోసం కెమెరా యాక్సెస్ మద్దతు లేదు.",
  "error.visualContext2DContext": "విజువల్ కాంటెక్స్ట్ కోసం 2D కాంటెక్స్ట్ పొందలేకపోయింది.",
  "error.snapshot2DContext": "స్నాప్‌షాట్ కోసం 2D కాంటెక్స్ట్ పొందలేకపోయింది.",
  "error.visualContextCaptureFailedPermission": "విజువల్ కాంటెక్స్ట్ విఫలమైంది: కెమెరా అనుమతి నిరాకరించబడింది.",
  "error.snapshotCaptureFailedPermission": "స్నాప్‌షాట్ విఫలమైంది: కెమెరా అనుమతి నిరాకరించబడింది.",
  "error.visualContextCaptureFailedNotFound": "విజువల్ కాంటెక్స్ట్ విఫలమైంది: కెమెరా కనుగొనబడలేదు.",
  "error.snapshotCaptureFailedNotFound": "స్నాప్‌షాట్ విఫలమైంది: కెమెరా కనుగొనబడలేదు.",
  "error.visualContextCaptureFailedNotReady": "విజువల్ కాంటెక్స్ట్ విఫలమైంది: కెమెరా సిద్ధంగా లేదు లేదా ఫీడ్‌లో సమస్య. {details}",
  "error.snapshotCaptureFailedNotReady": "స్నాప్‌షాట్ విఫలమైంది: కెమెరా సిద్ధంగా లేదు లేదా ఫీడ్‌లో సమస్య. {details}",
  "error.visualContextCaptureFailedGeneric": "విజువల్ కాంటెక్స్ట్ విఫలమైంది: {details}",
  "error.snapshotCaptureFailedGeneric": "స్నాప్‌షాట్ విఫలమైంది: {details}",

  // New UI strings
  "app.loading": "యాప్ లోడ్ అవుతోంది...",
  "audio.loading": "ఆడియో లోడ్ అవుతోంది",
  "audio.pause": "పాజ్",
  "audio.play": "ప్లే",
  "audio.progress": "ఆడియో పురోగతి",
  "camera.observerActive": "పరిశీలకుడు సక్రియంగా ఉన్నారు",
  "chat.bookmark.cancel": "రద్దు చేయండి",
  "chat.bookmark.editAria": "బుక్‌మార్క్ సారాంశాన్ని సవరించండి",
  "chat.bookmark.hiddenBelow": "సందేశాలు క్రింద దాచబడ్డాయి",
  "chat.bookmark.hiddenCount": "పైన {count} సందేశాలు దాచబడ్డాయి",
  "chat.bookmark.returnToRecent": "క్రింద ఉన్న సందేశాలకు వెళ్ళు",
  "chat.bookmark.save": "సేవ్ చేయండి",
  "chat.bookmark.showAboveChunk": "పైన ఉన్న సందేశాలు చూపించు",
  "chat.bookmark.showNextChunk": "పైన {count} చూపించు",
  "chat.bookmark.showPreviousChunk": "క్రింద {count} చూపించు",
  "chat.bookmark.showingAboveRange": "{total} లో {start}-{end} పైన ఉన్న సందేశాలు చూపిస్తోంది",
  "chat.bookmark.summaryPlaceholder": "సారాంశం...",
  "chat.camera.capturePhoto": "ఫోటో తీయండి",
  "chat.camera.startRecording": "వీడియో రికార్డింగ్ ప్రారంభించండి",
  "chat.deleteMessage": "సందేశాన్ని తొలగించండి",
  "chat.drawMessage": "సందేశాన్ని గీయండి",
  "chat.focusedMode.enter": "ఫోకస్డ్ మోడ్‌లోకి వెళ్ళండి",
  "chat.focusedMode.exit": "ఫోకస్డ్ మోడ్ నుండి నిష్క్రమించండి",
  "chat.music.finalizing": "సంగీతాన్ని సేవ్ చేస్తోంది...",
  "chat.music.starting": "సంగీతాన్ని ప్రారంభిస్తోంది...",
  "chat.music.streaming": "సంగీతాన్ని స్ట్రీమ్ చేస్తోంది...",
  "chat.pdf.loadFailed": "PDF లోడ్ చేయడం విఫలమైంది",
  "debugLog.clearLogs": "లాగ్‌లను క్లియర్ చేయండి",
  "debugLog.clearUploads": "ప్రస్తుత API కీ కోసం అన్ని Gemini అప్‌లోడ్‌లను తొలగించండి",
  "debugLog.close": "ముగించు",
  "debugLog.error": "లోపం",
  "debugLog.noTraffic": "ఇంకా ట్రాఫిక్ రికార్డ్ చేయలేదు.",
  "debugLog.requestPayload": "రిక్వెస్ట్ పేలోడ్",
  "debugLog.response": "రెస్పాన్స్",
  "debugLog.title": "ట్రాఫిక్ లాగ్",
  "error.cameraOverconstrained": "అభ్యర్థించిన సెట్టింగ్‌లతో కెమెరా అందుబాటులో లేదు. {errorMessage}",
  "globalProfile.ariaLabel": "గ్లోబల్ ప్రొఫైల్ సారాంశం",
  "globalProfile.loading": "ప్రొఫైల్ లోడ్ అవుతోంది...",
  "globalProfile.noProfile": "ఇంకా ప్రొఫైల్ లేదు.",
  "header.apiKey": "API కీ",
  "header.apiKeyRequired": "API కీ అవసరం",
  "header.clickToViewStatus": "స్థితిని చూడటానికి క్లిక్ చేయండి",
  "header.manageApiKey": "API కీ నిర్వహించండి",
  "header.pauseMaestro": "Maestro పాజ్ చేయండి",
  "header.resumeMaestro": "Maestro పునఃప్రారంభించండి",
  "header.viewTrafficLogs": "ట్రాఫిక్ లాగ్‌లను చూడండి",
  "miniGame.codeEmpty": "మినీ-గేమ్ కోడ్ ఖాళీగా ఉంది.",
  "miniGame.failedToRender": "మినీ-గేమ్ రెండర్ చేయడం విఫలమైంది.",
  "miniGame.fullscreen": "ఫుల్ స్క్రీన్",
  "miniGame.hideCode": "కోడ్ దాచు",
  "miniGame.launching": "మినీ-గేమ్ లాంచ్ అవుతోంది...",
  "miniGame.restart": "రీస్టార్ట్",
  "miniGame.retry": "మళ్ళీ ప్రయత్నించండి",
  "miniGame.runtimeError": "మినీ-గేమ్ లోపం: {error}",
  "miniGame.showCode": "కోడ్ చూపించు",
  "miniGame.title": "మినీ గేమ్",
  "miniGame.titleWithFile": "మినీ గేమ్ {fileName}",
  "officeFile.failedToParse": "ఇన్‌లైన్ ప్రివ్యూ పార్స్ చేయడం విఫలమైంది.",
  "officeFile.localPreviewUnavailable": "లోకల్ ప్రివ్యూ అందుబాటులో లేదు. స్థానికంగా తెరవడానికి తిరిగి అటాచ్ చేయండి.",
  "officeFile.openFile": "ఫైల్ తెరవండి",
  "officeFile.openInGoogleWorkspace": "Google Workspace లో తెరవండి",
  "officeFile.parsingInlinePreview": "ఇన్‌లైన్ ప్రివ్యూ పార్స్ చేస్తోంది...",
  "officeFile.parsingPreview": "ప్రివ్యూ పార్స్ చేస్తోంది...",
  "officeFile.previewUnavailable": "ఈ ఫైల్‌కు ప్రివ్యూ అందుబాటులో లేదు.",
  "officeFile.rawExtractedText": "రా ఎక్స్‌ట్రాక్టెడ్ టెక్స్ట్",
  "sessionControls.customizeColors": "రంగులను కస్టమైజ్ చేయండి",
  "streaming.connectingTo": "{model}కి కనెక్ట్ అవుతోంది (ప్రయత్నం {attempt}/{total})...",
  "streaming.failedToPrepareAttachment": "పంపడానికి ఇటీవలి అటాచ్‌మెంట్ \"{name}\" సిద్ధం చేయడం విఫలమైంది. మళ్ళీ ప్రయత్నించండి లేదా ఫైల్‌ను మళ్ళీ అటాచ్ చేయండి.",
  "streaming.failedToPrepareCurrent": "అటాచ్ చేసిన మీడియా \"{name}\" Gemini కోసం సిద్ధం చేయడం విఫలమైంది. మళ్ళీ ప్రయత్నించండి లేదా ఫైల్‌ను మళ్ళీ అటాచ్ చేయండి.",
  "streaming.highDemand": "అధిక డిమాండ్ గుర్తించబడింది. అభ్యర్థన Google సర్వర్‌లలో క్యూలో ఉంది.",
  "streaming.noOutputAfter": "{seconds}s తర్వాత మోడల్ అవుట్‌పుట్ లేదు. అభ్యర్థన Google సర్వర్‌లలో క్యూలో ఉండవచ్చు.",
  "streaming.phaseConnecting": "కనెక్ట్ అవుతోంది",
  "streaming.phaseFinalResponse": "చివరి స్పందన",
  "streaming.phaseFinalizing": "పూర్తి చేస్తోంది",
  "streaming.phaseHighDemand": "అధిక డిమాండ్",
  "streaming.phasePreparingRequest": "అభ్యర్థన సిద్ధం చేస్తోంది",
  "streaming.phaseProcessing": "ప్రాసెస్ చేస్తోంది",
  "streaming.phaseRetrying": "మళ్ళీ ప్రయత్నిస్తోంది",
  "streaming.phaseSwitchingModel": "మోడల్ మారుస్తోంది",
  "streaming.phaseThinking": "ఆలోచిస్తోంది",
  "streaming.preparingRequestContext": "అభ్యర్థన కాంటెక్స్ట్ సిద్ధం చేస్తోంది...",
  "streaming.retryingIn": "{seconds}s లో మళ్ళీ ప్రయత్నిస్తోంది...",
  "streaming.switchingFallback": "ఫాల్‌బ్యాక్ మోడల్ {model}కి మారుతోంది.",
  "streaming.waitingForOutput": "మోడల్ అవుట్‌పుట్ కోసం వేచి ఉంది... {seconds}s గడిచింది.",
  "textFile.rawText": "రా టెక్స్ట్",
  "textFile.unableToDecode": "టెక్స్ట్ అటాచ్‌మెంట్ డీకోడ్ చేయలేకపోయింది.",
  "themeCustomizer.cancel": "రద్దు చేయండి",
  "themeCustomizer.confirm": "ధృవీకరించండి",
  "themeCustomizer.deletePreset": "సేవ్ చేసిన ప్రీసెట్‌ను తొలగించండి",
  "themeCustomizer.exportAs": "ఇలా ఎగుమతి చేయండి",
  "themeCustomizer.exportToFile": "ఫైల్‌కు ఎగుమతి చేయండి",
  "themeCustomizer.importFromFile": "ఫైల్ నుండి దిగుమతి చేయండి",
  "themeCustomizer.instruction": "ఒక రంగు వృత్తాన్ని నొక్కి, పిక్కర్‌లో లాగండి. మార్పులు వెంటనే వర్తించబడతాయి.",
  "themeCustomizer.namePlaceholder": "థీమ్ పేరు...",
  "themeCustomizer.quickThemes": "త్వరిత థీమ్‌లు",
  "themeCustomizer.resetAll": "అన్నీ రీసెట్ చేయండి",
  "themeCustomizer.saveAs": "ఇలా సేవ్ చేయండి",
  "themeCustomizer.saveAsPreset": "ప్రీసెట్‌గా సేవ్ చేయండి",
  "themeCustomizer.storeDescription": "మరిన్ని థీమ్‌లను బ్రౌజ్ చేసి కొనుగోలు చేయండి",
  "themeCustomizer.storeTitle": "థీమ్ స్టోర్",
  "themeCustomizer.title": "రంగులు మార్చండి",
  "themeStore.buy": "కొనుగోలు",
  "themeStore.close": "ముగించు",
  "themeStore.footerNote": "కొనుగోళ్ళు Google Play ద్వారా నిర్వహించబడతాయి మరియు మీ Google ఖాతాతో ముడిపడి ఉంటాయి. రీఇన్‌స్టాల్‌లో గత కొనుగోళ్ళను తిరిగి పొందడానికి రీస్టోర్ బటన్ (↩) నొక్కండి.",
  "themeStore.loading": "థీమ్‌లు లోడ్ అవుతున్నాయి…",
  "themeStore.notAvailable": "థీమ్ కొనుగోళ్ళు Google Play ద్వారా Android లో మాత్రమే అందుబాటులో ఉంటాయి.",
  "themeStore.owned": "సొంతమైంది",
  "themeStore.purchaseFailed": "కొనుగోలు విఫలమైంది. దయచేసి మీ కనెక్షన్ తనిఖీ చేసి మళ్ళీ ప్రయత్నించండి.",
  "themeStore.restorePurchases": "కొనుగోళ్ళు పునరుద్ధరించండి",
  "themeStore.title": "థీమ్ స్టోర్",
};