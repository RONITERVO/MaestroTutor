...
export const ruTranslations: Record<string, string> = {
  ...
  "apiKeyGate.testAppLink": " Отправили данные и получили подтверждение на почту? Скачайте приложение здесь: https://play.google.com/store/apps/details?id=com.ronitervo.maestrotutor",
  "apiKeyGate.testerFormDescription": "Введите email, который вы используете для Google Play Store, чтобы получить приглашение в нашу бета-версию для Android!",
  "apiKeyGate.testerFormSubmit": "Получить ранний доступ",
  "apiKeyGate.testerFormSubmitting": "Отправка...",
  "apiKeyGate.testerFormSuccessTitle": "Спасибо!",
  "apiKeyGate.testerFormSuccessDesc": "Живые люди проверяют ваш email. Это займет некоторое время. Ссылка не заработает для {TheUserEmail} мгновенно.",
  "apiKeyGate.testerFormNextSteps": "Когда подтверждение будет завершено (до 1 дня), мы отправим вам эту ссылку на почту (вы можете пока закрыть эту страницу): ",
  "apiKeyGate.testerFormError": "Что-то пошло не так. Пожалуйста, попробуйте снова.",
