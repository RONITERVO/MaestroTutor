...
const ColorGroupSection: React.FC<ColorGroupSectionProps> = ({
  ...
  return (
    <div>
      ...
      >
        <span className={`text-[10px] text-faded-label transition-transform duration-200 ${isCollapsed ? '' : 'rotate-90'}`}>
          &#9654;
        </span>
        <span className="text-xs font-hand text-faded-label uppercase tracking-wider">
          {group.groupName}
        </span>
        {groupModifiedCount > 0 && (
          <span className="ml-auto text-[10px] bg-action-accent/20 text-action-accent px-1.5 py-0.5 rounded-full font-semibold">
            {groupModifiedCount}
          </span>
        )}
      </button>
      ...
      {group.groupDescription && !isCollapsed && (
        <p className="text-[10px] text-faded-label/70 ml-4 -mt-0.5 mb-1.5">
      ...
      {!isCollapsed && (
        <>
              ...
              return (
                <button
                  ...
                  className={`flex flex-col items-center gap-1 p-1.5 rounded-lg transition-all duration-150
                    ${active ? 'bg-hover-fill ring-2 ring-action-accent scale-105' : 'active:scale-95'}`}
                >
                  <div
                    className={`w-8 h-8 rounded-full border-2 shadow-sm transition-colors
                      ${modified ? 'border-action-accent' : 'border-ui-border'}`}
                    style={{ backgroundColor: hex }}
                  />
                  <span className="text-[10px] text-faded-label leading-tight text-center truncate w-full">
                    {cv.friendlyName}
                  </span>
                  {modified && (
                    <div className="w-1.5 h-1.5 rounded-full bg-action-accent -mt-0.5" />
                  )}
                </button>
              );
            })}
          ...
          {activeColor && activeColorVar && (
            <div className="mt-3 p-3 bg-hover-fill/50 rounded-lg border border-ui-border/50 animate-fade-up">
              <div className="flex items-center justify-between mb-1">
                ...
                {isModified(activeColorVar) && (
                  <button
                    ...
                    className="flex items-center gap-1 text-xs text-faded-label hover:text-foreground transition-colors"
                  >
              ...
              {activeColor.description && (
                <p className="text-[11px] text-faded-label/80 mb-2 leading-snug">
                  {activeColor.description}
                </p>
              )}
              <HexColorPicker
              ...
              />
              <div className="mt-2 text-xs text-faded-label text-center font-mono">
...
const ThemeCustomizerPanel: React.FC<ThemeCustomizerPanelProps> = ({ onClose }) => {
  ...
  return (
    <>
      ...
      {/* Panel */}
      <div
        className="fixed inset-x-0 bottom-0 z-[90] flex flex-col bg-panel-surface/10 backdrop-blur-md border-t border-ui-border shadow-2xl rounded-t-2xl overflow-hidden"
        style={{ height: '45vh', maxHeight: '45vh' }}
      >
        {/* Header */}
        <div className="flex flex-col items-center pt-2 pb-1 px-4 shrink-0">
          <div className="w-10 h-1 bg-faded-label/30 rounded-full mb-2" />
            ...
            <div className="flex items-center gap-1">
              {hasAnyCustomization && (
                <button
                  ...
                  className="flex items-center gap-1 text-xs text-faded-label hover:text-foreground px-2 py-1 rounded transition-colors"
              ...
              <button
                type="button"
                onClick={onClose}
                className="p-1.5 text-faded-label hover:text-foreground rounded-full transition-colors"
              >
                <IconXMark className="w-5 h-5" />
              </button>
            </div>
          </div>
          <p className="w-full text-[11px] text-faded-label/85 mt-0.5">
          ...
          {namingMode && (
            <div className="flex w-full items-center gap-1.5 mt-1.5 animate-fade-up">
              <span className="text-[10px] font-hand text-faded-label uppercase tracking-wider shrink-0">
              ...
              <input
                ...
                className="flex-1 bg-panel-surface/80 border border-sketch-ring/30 sketchy-border-thin px-2 py-0.5 text-xs text-foreground placeholder-faded-label/60 focus:outline-none focus:border-action-accent transition-colors font-hand"
              ...
              <button
                ...
                className="p-1 text-action-accent hover:text-foreground disabled:opacity-30 transition-colors"
                title="Confirm"
              >
              ...
              <button
                ...
                className="p-1 text-faded-label hover:text-foreground transition-colors"
                title="Cancel"
              >
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-8 space-y-4 scrollbar-hide">
          ...
          {/* ── Quick Themes ────────────────────────── */}
          <div>
            <h3 className="text-xs font-hand text-faded-label uppercase tracking-wider mb-2">
              Quick Themes
            </h3>
              ...
              {PRESET_THEMES.map(preset => {
                ...
                return (
                  <button
                    ...
                    className="flex items-center gap-2 px-3 py-2 rounded-lg border border-ui-border/60 hover:border-action-accent/50 bg-panel-surface/60 hover:bg-hover-fill/50 transition-all active:scale-95"
                  >
                    {/* 3-dot preview swatch */}
                    <div className="flex gap-0.5">
                      <div className="w-3 h-3 rounded-full border border-ui-border/40" style={{ backgroundColor: hslStringToHex(previewBg) }} />
                      <div className="w-3 h-3 rounded-full border border-ui-border/40" style={{ backgroundColor: hslStringToHex(previewAccent) }} />
                      <div className="w-3 h-3 rounded-full border border-ui-border/40" style={{ backgroundColor: hslStringToHex(previewFg) }} />
                    </div>
                    <div className="text-left">
                      <div className="text-xs font-semibold text-foreground leading-tight">
                        {preset.name}
                      </div>
                      <div className="text-[9px] text-faded-label leading-tight">
                        {preset.description}
                      </div>
                    </div>
                  </button>
                ...
                return (
                  <div key={`saved-${idx}`} className="relative group/saved">
                    <button
                      type="button"
                      onClick={() => applyPreset(preset)}
                      className="flex items-center gap-2 px-3 py-2 rounded-lg border border-action-accent/30 hover:border-action-accent/50 bg-panel-surface/60 hover:bg-hover-fill/50 transition-all active:scale-95"
                    >
                      <div className="flex gap-0.5">
                        <div className="w-3 h-3 rounded-full border border-ui-border/40" style={{ backgroundColor: hslStringToHex(previewBg) }} />
                        <div className="w-3 h-3 rounded-full border border-ui-border/40" style={{ backgroundColor: hslStringToHex(previewAccent) }} />
                    ...
                    <button
                      ...
                      className="absolute -top-1.5 -right-1.5 w-4 h-4 rounded-full bg-danger-color text-danger-color-text flex items-center justify-center text-[10px] leading-none opacity-0 group-hover/saved:opacity-100 transition-opacity"
          ...
          {/* ── Divider ────────────────────────── */}
          <div className="border-t border-ui-border/30" />
