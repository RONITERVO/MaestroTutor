...
const BookmarkActions: React.FC<BookmarkActionsProps> = ({ t, message, maxVisibleMessages, onChangeMaxVisibleMessages, updateMessage }) => {
  ...
  const containerClasses = "flex items-center gap-1 px-1.5 py-0.5 bg-action-accent/90 text-action-accent-text sketchy-border-thin shadow-sm";
    ...
    return (
      <div className={containerClasses} role="region" aria-label="Edit bookmark summary" onPointerDown={e => e.stopPropagation()} onClick={e => e.stopPropagation()}>
        <input
          ...
          className="bg-panel-surface/80 border border-sketch-ring/30 sketch-shape-0 px-2 py-0.5 text-xs text-foreground placeholder-faded-label/60 focus:outline-none focus:border-action-accent focus:bg-panel-surface w-48 transition-colors font-hand"
          placeholder="Summary..."
          autoFocus
        />
        <button
          type="button"
          onClick={save}
          className="p-1 sketch-shape-2 hover:bg-action-accent/50 text-action-accent-text transition-colors"
          title="Save"
        >
        ...
        <button
          type="button"
          onClick={cancel}
          className="p-1 sketch-shape-4 hover:bg-action-accent/50 text-action-accent-text transition-colors"
          title="Cancel"
        >
  ...
  return (
    ...
    >
      <button
        ...
        className="px-2 py-0.5 text-xs font-hand sketch-shape-6 hover:bg-action-accent/30 disabled:opacity-50"
      ...
      <button
        ...
        className="px-2 py-0.5 text-xs font-hand sketch-shape-8 hover:bg-action-accent/30 disabled:opacity-50"
        aria-label={t('chat.bookmark.incrementAria') || 'Increase maximum visible messages'}
        title={t('chat.bookmark.incrementTitle') || 'Increase'}
      >
        +
      </button>
      <div className="w-px h-4 bg-sketch-ring/40 mx-1" aria-hidden />
      <button
        type="button"
        onClick={startEditing}
        className="px-1.5 py-0.5 sketch-shape-10 hover:bg-action-accent/30"
