...
type BrowserOptions<T> = Omit<T, 'aiClient' | 'resolveAiClient' | 'onUsage'> & { aiClient?: CoreGeminiClient };
...
export const runReplySuggestions = (input: ReplySuggestionsInput, options: BrowserOptions<ReplySuggestionsOptions> = {}) =>
  runSuggestions(input, { ...options, ...browserClientSource(options.aiClient) });
